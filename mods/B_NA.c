...
#define _f_rate _f_rate__B_Na 
#define rate rate__B_Na 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "alp_B_Na", _hoc_alp,
 "bet_B_Na", _hoc_bet,
 "expM1_B_Na", _hoc_expM1,
 "rate_B_Na", _hoc_rate,
 "states_B_Na", _hoc_states,
 0, 0
};
#define alp alp_B_Na
#define bet bet_B_Na
#define expM1 expM1_B_Na
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 2, "na_ion");
 	...
 	ivoc_help("help ?1 B_Na C:/Users/mazar/Desktop/AIS_MODEL/B_NA.mod\n");
...
double alp (  double _lv , double _li ) {
 ...
 _lv = _lv ;
   ...
   if ( _li  == 0.0 ) {
     _lalp = _lq10 * .182 * expM1 ( _threadargscomma_ - _lv + 2.0 - 35.0 , 9.0 ) ;
     }
   else if ( _li  == 1.0 ) {
     _lalp = _lq10 * 1.0 * ( 0.061 * expM1 ( _threadargscomma_ - _lv + 2.0 - 48.0 , 3.0 ) + 0.0166 ) ;
     }
...
double bet (  double _lv , double _li ) {
 ...
 _lv = _lv ;
   _lq10 = pow( 3.0 , ( ( celsius - 23.0 ) / 10.0 ) ) ;
   if ( _li  == 0.0 ) {
     _lbet = _lq10 * .124 * expM1 ( _threadargscomma_ _lv - 2.0 + 35.0 , 9.0 ) ;
     }
   else if ( _li  == 1.0 ) {
     _lbet = _lq10 * 1.0 * .0018 * expM1 ( _threadargscomma_ _lv - 2.0 + 84.0 , 18.0 ) ;
     }
...
static int  _f_rate (  double _lv ) {
 ...
 {int  _li ;for ( _li = 0 ; _li <= 1 ; _li ++ ) {
     ...
     if ( ((double) _li )  == 0.0 ) {
       ...
       inf [ _li ] = 1.0 / ( 1.0 + exp ( ( _lv + 75.0 - 0.0 ) / 9.0 ) ) ;
       }
...
static int _ode_count(int _type){ hoc_execerror("B_Na", "cannot be used with CVODE"); return 0;}
 ...
 v=_v;
{
  ena = _ion_ena;
 { error =  states();
 if(error){fprintf(stderr,"at line 49 in file B_NA.mod:\n	SOLVE states\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "B_NA.mod";
static const char* nmodl_file_text = 
