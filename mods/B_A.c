...
#define _f_rate _f_rate__B_A 
#define rate rate__B_A 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "alp_B_A", _hoc_alp,
 "bet_B_A", _hoc_bet,
 "expM1_B_A", _hoc_expM1,
 "rate_B_A", _hoc_rate,
 "states_B_A", _hoc_states,
 0, 0
};
#define alp alp_B_A
#define bet bet_B_A
#define expM1 expM1_B_A
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 2, "k_ion");
 	...
 	ivoc_help("help ?1 B_A C:/Users/mazar/Desktop/AIS_MODEL/B_A.mod\n");
...
double alp (  double _lv , double _li ) {
 ...
 _lv = _lv ;
   ...
   if ( _li  == 0.0 ) {
     _lalp = _lq10 * .032 * expM1 ( _threadargscomma_ - _lv - 64.0 - 0.0 , 6.0 ) ;
     }
   else if ( _li  == 1.0 ) {
     _lalp = _lq10 * 0.05 / ( exp ( ( _lv + 86.0 + 0.0 ) / 10.0 ) + 1.0 ) ;
     }
...
double bet (  double _lv , double _li ) {
 ...
 _lv = _lv ;
   _lq10 = pow( 3.0 , ( ( celsius - 23.0 ) / 10.0 ) ) ;
   if ( _li  == 0.0 ) {
     _lbet = _lq10 * 0.203 * exp ( ( - _lv - 40.0 - 0.0 ) / 24.0 ) ;
     }
   else if ( _li  == 1.0 ) {
     _lbet = _lq10 * 0.05 / ( exp ( ( - _lv - 86.0 - 0.0 ) / 10.0 ) + 1.0 ) ;
     }
...
static int _ode_count(int _type){ hoc_execerror("B_A", "cannot be used with CVODE"); return 0;}
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 51 in file B_A.mod:\n	SOLVE states\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "B_A.mod";
static const char* nmodl_file_text = 
